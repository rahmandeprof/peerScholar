...
import { APIErrorExceptionFilter } from '@/app/auth/api-error-exception-filter';
import { SkipBodyParsingMiddleware } from '@/app/auth/middlewares';
import {
	AFTER_HOOK_KEY,
	AUTH_INSTANCE_KEY,
...
export class AuthModule implements NestModule, OnModuleInit {
	...
	configure(consumer: MiddlewareConsumer) {
		...
		let basePath = this.auth.options.basePath ?? '/v1/auth';
	...
	private setupHooks(providerMethod: (ctx: unknown) => Promise<void>) {
		if (!this.auth.options.hooks) return;
	 ...
	 */
	static forRoot(auth: Auth, options: AuthModuleOptions = {}) {
